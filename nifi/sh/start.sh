...
prop_replace 'nifi.cluster.load.balance.host'   "${NIFI_CLUSTER_LOAD_BALANCE_HOST:-$HOSTNAME}"
...
case ${AUTH} in
    ...
    oidc)
        ...
        if [ -n "${CA_SERVER_HOST}" ] && [ -n "${CA_SERVER_PORT}" ] && [ -n "${CA_TOKEN}" ]; then

            # OIDC doesn't have any login identitiy provider configured
            prop_replace 'nifi.security.user.login.identity.provider'   ""
            prop_replace 'nifi.security.user.authorizer'                "${NIFI_SECURITY_USER_AUTHORIZER:-managed-authorizer}"

            . "${scripts_dir}/generate_cert.sh"
            . "${scripts_dir}/oidc.sh"
        else
            echo 'Found invalid settings for OIDC mechanism, Required ==> [CA_SERVER_HOST, CA_SERVER_PORT, CA_TOKEN]. Instead will use Single User Credentials mechanism'
        fi
        ;;
esac
